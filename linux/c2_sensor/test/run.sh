...
# ===============================================================================
# Tier 0 - Pure-Python algorithm + transmission-layer tests (pytest, no containers)
# Tier 1 - Rust workspace build/compile validation
#
# Usage:
...
if $RUN_TIER0; then
    echo "=== Tier 0: algorithm + transmission-layer tests (pytest) ==="
    python3 -m pytest "$SCRIPT_DIR/tier0" -v --tb=short -p no:cacheprovider || OVERALL=1
fi

if $RUN_TIER1; then
    echo ""
    echo "=== Tier 1: Rust workspace build/compile validation (cargo check) ==="
