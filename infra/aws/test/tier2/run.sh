...
IMAGE_TAG="nexus-aws-connectors-tier2:latest"
NO_CACHE=""
...
echo "[tier2] Building IaC-validation image ($ENGINE, context=$AWS_DIR)..."
"$ENGINE" build $NO_CACHE -f "$SCRIPT_DIR/Dockerfile" -t "$IMAGE_TAG" "$AWS_DIR"

echo "[tier2] Running deploy/ IaC validation (terraform + moto + scanners) inside container..."
