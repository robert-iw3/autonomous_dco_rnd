"""
Tier-0 -- Transmission-layer conformance for the Nexus AWS connectors
...
SECRET = b"aws-connector-tier0-integrity-secret"
SENSOR_ID = sensor_id_for("123456789012", "lab", "us-east-1")
...
def _reference_hmac(payload: bytes, sequence: int, sensor_id: str, ts: int) -> str:
    ...
    cross-checked against aws_connectors_logic_mirror.compute_hmac (itself
...
class TestComputeHmacContract:
    def test_matches_independent_reference(self):
        payload = b"synthetic-aws-connector-parquet-batch"
    ...
    def test_byte_order_is_payload_then_be64_seq_then_id_then_be64_ts(self):
        ...
        and every other Nexus sensor. Confirm our mirror diverges from a
        plausible-but-wrong alternative ordering (id-first, LE timestamps)."""
        payload = b"order-sensitive-aws-batch-bytes"
    ...
    def test_tampered_payload_fails_recheck(self):
        payload = b"original-aws-connector-batch-bytes"
        seq, ts = 7, 1_700_000_500
        digest = compute_hmac(SECRET, payload, seq, SENSOR_ID, ts)
        assert compute_hmac(SECRET, b"tampered-aws-connector-batch-bytes!!", seq, SENSOR_ID, ts) != digest
...
class TestRequiredHeaderContract:
    ...
    def test_real_source_sends_exactly_these_seven_headers_via_bearer_auth_and_header_calls(self, connector_dir):
        ...
        .bearer_auth() / read an auth_token -- so Authorization was absent and
...
# same way transmit_bytes() builds its real request (transmitter.rs:277-288).
# -----------------------------------------------------------------------------
...
def _post_synthetic_batch(url, payload, sequence, ts, sensor_type, token="tier0-test-token"):
...
class TestEndToEndTransmission:
    @pytest.mark.parametrize("connector", ["vpc", "cloudtrail", "guardduty"])
    def test_synthetic_batch_reaches_mock_ingress_with_valid_contract(self, mock_ingress, connector):
        payload = b"\x50\x41\x52\x31synthetic-aws-" + connector.encode() + b"-parquet-bytes"
    ...
    def test_tampered_in_flight_payload_breaks_hmac_recheck(self, mock_ingress):
        ...
        _post_synthetic_batch(mock_ingress, payload, sequence, ts, WIRE_SENSOR_TYPE["vpc"])
        captured = _CapturingHandler.captured

        digest = compute_hmac(SECRET, captured["body"], sequence, SENSOR_ID, ts)
        recheck = compute_hmac(SECRET, b"mutated-aws-connector-batch-bytes!", sequence, SENSOR_ID, ts)
    ...
    def test_sensor_id_binds_into_hmac_so_spoofed_identity_fails(self, mock_ingress):
        ...
        connector from impersonating a different AWS account/region pairing."""
        payload = b"impersonation-attempt-batch-bytes"
        sequence, ts = 5, 1_700_000_400
        digest = compute_hmac(SECRET, payload, sequence, SENSOR_ID, ts)

        spoofed_id = sensor_id_for("999999999999", "prod", "eu-west-1")
    ...
    def test_wire_sensor_type_distinguishes_connectors_at_the_mock_ingress(self, mock_ingress):
        ...
        seen = set()
        for i, connector in enumerate(("vpc", "cloudtrail", "guardduty"), start=10):
