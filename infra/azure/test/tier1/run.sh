...
IMAGE_TAG="nexus-azure-connectors-tier1:latest"
NO_CACHE=""
...
echo "[tier1] Building cargo-check image ($ENGINE, context=$AZURE_DIR)..."
"$ENGINE" build $NO_CACHE -f "$SCRIPT_DIR/Dockerfile" -t "$IMAGE_TAG" "$AZURE_DIR"

echo "[tier1] Running 'cargo check' for nsg, activity, entraid inside container..."
