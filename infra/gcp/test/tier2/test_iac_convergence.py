"""
Tier-2 -- Terraform convergence checks for the GCP deploy/ stacks.

  - `terraform init` + `terraform validate`:  runs in CI (no credentials needed).
  - `terraform fmt -check`:                   style gate.
  - `terraform apply`:                        deferred to gated real-GCP run.
"""
import os
import shutil
import subprocess
