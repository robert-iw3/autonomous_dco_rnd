...
IMAGE_TAG="nexus-gcp-connectors-tier1:latest"
NO_CACHE=""
...
echo "[tier1] Building cargo-check image ($ENGINE, context=$GCP_DIR)..."
"$ENGINE" build $NO_CACHE -f "$SCRIPT_DIR/Dockerfile" -t "$IMAGE_TAG" "$GCP_DIR"

echo "[tier1] Running 'cargo check' for audit, scc, vpc inside container..."
