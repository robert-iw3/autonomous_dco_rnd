//! Improved transmitter shared across connectors.
//!
//! Changes over the original per-connector `transmitter.rs`:
//!   1. BOUNDED SPOOL. The original wrote `batch_{seq}.parquet` to the spool
//!      dir and only deleted it on success, but never enforced
//!      `max_spool_files` / `max_spool_bytes` (those config fields existed in
//!      the VPC connector and were dead). Under sustained gateway failure the
//!      pod's ephemeral volume fills and the pod is evicted. We now evict the
//!      oldest spool files before writing a new one.
//!   2. OPTIONAL STARTUP REPLAY. The original spool was write-only: nothing
//!      ever re-read it, so it provided no actual recovery. For transports
//!      with NO upstream durability (syslog), set `spool_replay = true` to
//!      resend spooled batches on boot. For queue-backed transports
//!      (SQS/Pub/Sub) leave it FALSE -- the queue is the source of truth and
//!      replaying would duplicate data.
//!   3. `n` (unused) removed.
//!
//! The HMAC construction and header set are unchanged and remain
//! wire-compatible with the existing connectors and the Axum gateway.
...
impl Transmitter {
    ...
    fn to_parquet(records: &[UnifiedFlowRecord]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        ...
        let batch = RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(Int32Array::from(records.iter().map(|r| r.pid).collect::<Vec<_>>())),
                Arc::new(Int32Array::from(records.iter().map(|r| r.uid).collect::<Vec<_>>())),
    ...
    /// Enforce spool bounds by evicting the oldest `batch_*.parquet` files
    /// until both the file-count and byte-size budgets are satisfied.
    ...
    /// Resend any spooled batches left over from a previous crash. Only call
    /// this when the transport has no upstream durability (e.g. syslog). For
    /// queue-backed transports this would duplicate data.
    ...
    /// Core send path: claims a fresh sequence, signs, and POSTs with backoff.
    /// On 2xx, removes `spool_path` (if any) and returns true.
