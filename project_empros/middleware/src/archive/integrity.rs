// Three-tier integrity verification for inbound Parquet payloads.
// Reuses the canonical HMAC protocol shared across all sensors.
...
impl std::fmt::Display for IntegrityViolation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::SequenceGap { expected_min, received } => write!(f, "Sequence gap: expected>{} got={}", expected_min, received),
            Self::SequenceReplay { sequence } => write!(f, "Replay: seq={}", sequence),
            Self::TemporalDrift { delta_secs } => write!(f, "Drift: {}s", delta_secs),
            Self::CrossOsCollision { sensor_type, offending_columns } => write!(f, "Cross-schema collision {}: {:?}", sensor_type, offending_columns),
            Self::SensorBanned { sensor_id } => write!(f, "Banned: {}", sensor_id),
        }
    }
}

struct SensorState { last_sequence: u64, seen: HashSet<u64>, order: Vec<u64>, failures: u32 }
impl SensorState {
    fn new() -> Self { Self { last_sequence: 0, seen: HashSet::with_capacity(REPLAY_WINDOW_SIZE), order: Vec::with_capacity(REPLAY_WINDOW_SIZE), failures: 0 } }
    fn record(&mut self, seq: u64) {
        if self.order.len() >= REPLAY_WINDOW_SIZE {
            if let Some(old) = self.order.first().copied() { self.seen.remove(&old); self.order.remove(0); }
        }
        self.seen.insert(seq); self.order.push(seq); self.last_sequence = seq;
    }
}
