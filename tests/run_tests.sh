...
# Change detection runs in the git work tree; the docker build context is the
# directory ABOVE it because every tests/Dockerfile.* COPYs autonomous_dco_rnd/<path>.
...
SECTIONS=(
    ...
    "memory|Dockerfile.memory|worker_memory + the DFIR platform projection contract"
...
TRIGGERS=(
    ...
    "integrations/dfir_platform/:memory"
    "tests/lab_memory_forensics|tests/lab_dfir_platform:memory"
...
# Returns non-zero when the diff cannot be taken at all, so the caller never reads
# "git failed" as "nothing changed".
get_changed_files() {
    cd "$REPO_ROOT" || return 1
    git rev-parse --is-inside-work-tree &>/dev/null || return 1
    if git rev-parse --verify "$BASE_REF" &>/dev/null; then
        git diff --name-only "$BASE_REF" HEAD || return 1
    fi
...
run_section() {
    ...
    local build_args=("-f" "${SCRIPT_DIR}/${dockerfile}" "-t" "${image}")
    [[ $REBUILD -eq 1 ]] && build_args+=("--no-cache")

    echo -e "${CYAN}[${name}]${RESET} building ${dockerfile}..."
    if ! docker build "${build_args[@]}" "$BUILD_CONTEXT" > "$log_file" 2>&1; then
...
# -- Build-context guard -------------------------------------------------------
# The build context is the parent of the repo, which on a developer box also holds
# every unrelated checkout. Without a .dockerignore there, docker uploads all of
# them to the daemon. Write the allow-list ourselves rather than refuse to run;
# only a context we cannot write to is fatal.

# Deny-all + re-admit: every tests/Dockerfile.* COPYs <repo>/<path>, and
# Dockerfile.sensors / Dockerfile.pipeline additionally COPY infra/, linux/ and
# windows/ straight from the context root, so those names stay admitted too.
write_context_ignore() {
    local dest="$1" repo_dir="$2"
    {
        echo "# Generated by ${repo_dir}/tests/run_tests.sh."
        echo "# This directory is the docker build context for the Nexus test images."
        echo "# generated-by: nexus run_tests.sh v2 -- regenerated when this stamp is stale."
        echo "# Deny everything, then re-admit only the trees the Dockerfiles COPY --"
        echo "# without this, every sibling checkout here is uploaded to the daemon."
        echo "*"
        echo "!${repo_dir}"
        echo "!infra"
        echo "!linux"
        echo "!windows"
        echo ""
        echo "# Rebuilt inside the images, or pure VCS/bytecode weight."
        echo "**/.git"
        echo "**/target"
        echo "**/__pycache__"
        echo "**/node_modules"
...
require_context_ignore() {
    if [[ "$BUILD_CONTEXT" == "$PROJECT_ROOT" ]]; then return 0; fi
    local ignore_file="${BUILD_CONTEXT}/.dockerignore" repo_dir
    repo_dir="$(basename "$PROJECT_ROOT")"
    # A hand-written .dockerignore is the operator's; only refresh our own stale stamp.
    if [[ -s "$ignore_file" ]]; then
        if ! grep -q "generated-by: nexus run_tests.sh" "$ignore_file"; then return 0; fi
        if grep -q "generated-by: nexus run_tests.sh v2" "$ignore_file"; then return 0; fi
    fi
    if ! write_context_ignore "$ignore_file" "$repo_dir"; then
        fail "Build context ${BUILD_CONTEXT} has no .dockerignore and one cannot be written"
        fail "there: a build would upload every sibling checkout to the docker daemon."
        fail "Refusing to build."
        echo "  Create it by hand, then re-run:"
        echo "    printf '%s\n' '*' '!${repo_dir}' > ${ignore_file}"
        exit 1
    fi
    log "Wrote ${ignore_file}: build context now admits ${repo_dir}/ (plus infra/, linux/, windows/) only."
}
...
case "$MODE" in
    ...
    explicit)
        ...
        ;;
    auto)
        # A broken diff must never look like "nothing to test" -- fall back to --full.
        if ! CHANGED_RAW=$(get_changed_files); then
            warn "Change detection failed in ${REPO_ROOT} (no git work tree, or '${BASE_REF}' unusable)."
            warn "Falling back to the FULL suite -- reporting success having run nothing is worse."
            MODE="full"
            for entry in "${SECTIONS[@]}"; do
                SECTIONS_TO_RUN+=("$(section_name "$entry")")
            done
        else
            CHANGED=$(printf '%s\n' "$CHANGED_RAW" | grep -v '^$' | sort -u || true)
            if [[ -z "$CHANGED" ]]; then
...
# Nothing gets built until the context is safe to hand to the daemon
require_context_ignore
